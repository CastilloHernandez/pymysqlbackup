...
class MysqlBackup:
	...
	def __init__(self, keep=90, databases=None, excludes=None, store=None, user="root", password=None, host=None):
		self.host = host
		self.keep = keep
	...
	def get_excludes(self):
		if self.excludes != None:
			return [s.strip() for s in self.excludes.strip().split(",")]
		else:
			return []
	...
	def backup(self):
		...
		skip.extend(excludes)
		for db in databases:
			if db in skip:
				logging.warning("Excluding db, %s." % db)
				continue
			...
			os.popen(dump_cmd)
			print "Dump db %s done." % db
			logging.info("Dump db %s done." % db)

"""
Prints out the usage for the command line.
"""
def usage():
	usage = ["mysqlbackup.py [-hkdebups]\n"]
	...
	usage.append("	[-e | --excludes] a comma separated list of databases to exclude\n")
...
def main(argv):
	...
	try:
		...
		opts, args = getopt.getopt(argv, "hn:k:d:e:t:u:p:s:", ["help", "keep=", "databases=", "excludes=", "store=", "user=", "password=", "host="])
	...
	try:
		if not (os.path.isdir(store)):
			os.makedirs(store)
		if not store.endswith(os.path.sep):
			store = store + os.path.sep
	except:
		logging.warning("El directorio destino no existe")
		sys.exit(errno.EIO)
	...
	try:
		...
		mysql_backup = MysqlBackup(keep, databases, excludes, store, user, password, host)
		mysql_backup.backup()
	except(Exception):
